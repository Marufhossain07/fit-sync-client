...
const Register = () => {
    ...
    const { createUser,googleSignIn, logOut, githubSignIn } = useContext(AuthContext);
    ...
    const handleError = (error) =>{
        if(error.pass){
            return toast.error("Password must contain at least 6 characters, one letter, one number, and one special character", {
                position: "top-center",
                autoClose: 2500,
                hideProgressBar: false,
                closeOnClick: true,
                ...
                <form onSubmit={handleSubmit(onSubmit,handleError)} className="space-y-6">
                    ...
                    <div className="relative my-6">
                        <input
                            ...
                            {...register("pass", { pattern: /^(?=.*[a-zA-Z])(?=.*\d)(?=.*[!@#$%&?]).{6,}$/ })}
